...
class pedidoController implements IApiControler
{
  ...
  public function ModificarUno($request, $response, $args)
  {
    ...
    if ($contadorModificaciones > 0 && $contadorModificaciones <= 5 && $id != null && $pedido != null) {
  ...
  public function terminarPedido($request, $response, $args)
  {
    ...
    if($respuesta) {
      ...
      if ($completo) {
        ...
        $newResponse = $response->withJson("Se prepararon todos los productos. Pedido listo para servir", 200);
      } 
      else {
        $newResponse = $response->withJson("Se finalizó la preparacion de los productos", 200);
      }
    } else {
      $newResponse = $response->withJson("No hay productos pendientes para este pedido", 200);
    }
    return $newResponse;
  }
