...
class encargadoController implements IApiControler
{
    ...
    public function CargarUno($request, $response, $args)
    {
        ...
        $encargadoNuevo->usuario = strtolower(substr($arrayDeParametros["nombre"], 0, 1)) . strtolower($arrayDeParametros["apellido"]);
    ...
    public function IniciarSesion($request, $response)
    {
        ...
        if (count($encargado) == 1 && $encargado[0]["clave"] == $arrayDeParametros["clave"]) {
            ...
            $newResponse = $response->withJson("No coinciden usuario y clave", 200);
        }
